...
from dqn.dqn import get_shared_dqn_config
from dqn.utils import create_env


def main():
    """Shared DQNの学習を実行するメイン関数。"""
    # MPE環境を指定
    ...
    config = get_shared_dqn_config(create_env(env_name))

    # 環境名をコンフィグに設定
    config.environment(env=env_name)

    # 結果の出力設定
    # ログは ~/ray_results/shared_dqn_simple_spread_v3 に保存される
    # TensorBoardで確認する場合: tensorboard --logdir ~/ray_results/shared_dqn_simple_spread_v3
    stop_conditions = {
    ...
    results = tune.run(
        "DQN",
        name="shared_dqn_simple_spread_v3",
